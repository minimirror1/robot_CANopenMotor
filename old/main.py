...
class CANopenNode(Node):
    def __init__(self, use_ros=True):
        # use_ros 속성을 가장 먼저 설정
        self.use_ros = use_ros  # 이 줄을 맨 앞으로 이동
        ...
        self.node = canopen.RemoteNode(2, eds_path)
        ...
        self.use_ros = use_ros  # ROS 사용 여부를 저장

    def get_logger(self):
        if self.use_ros:
            return super().get_logger()
        else:
            return DummyLogger()  # ROS를 사용하지 않을 때의 로거
...
def testmove(canopen_node):
    while True:
        # 목표 위치를 10000으로 설정
        target_position = 10000
        
        # 목표 위치 설정
        canopen_node.node.rpdo[1]['Controlword'].phys = 0x2f
        canopen_node.node.rpdo[1].transmit()

        canopen_node.node.rpdo[1]['Target Position'].phys = target_position
        canopen_node.node.rpdo[1]['Controlword'].phys = 0x3f
        canopen_node.node.rpdo[1].transmit()
...
def main(args=None):
    ...
    if use_ros:
        ...
        node = CANopenNode(use_ros=True)
        rclpy.spin(node)
        rclpy.shutdown()
    else:
        node = CANopenNode(use_ros=False)
        move_generator = testmove(node)
        try:
            while True:
                next(move_generator)
